...
<xsl:param name="parameter11"/>
<xsl:param name="parameter12"/>
<xsl:param name="parameter13"/>
<xsl:param name="parameter14"/>
<xsl:param name="parameter15"/>
...
<xsl:param name="reference11"/>
<xsl:param name="reference12"/>
<xsl:param name="reference13"/>
<xsl:param name="reference14"/>
<xsl:param name="reference15"/>
...
//		$oStmt = @PHPReportsDBI::db_query($oCon,trim($sSQL)) or $oError->showMsg("QUERYERROR");
		if(!($oStmt = @PHPReportsDBI::db_query($oCon,trim($sSQL)))){
//			echo mysql_error();
			$oError->showMsg("QUERYERROR");
		}
	}else
		$oStmt = $this->_oQuery;
...
<xsl:template name="PARAMETERS">
	...
	<xsl:if test="string-length($parameter11)>0">
		$oParameters[<xsl:value-of select="$reference11"/>]=&quot;<xsl:value-of select="$parameter11"/>&quot;;
	</xsl:if>
	<xsl:if test="string-length($parameter12)>0">
		$oParameters[<xsl:value-of select="$reference12"/>]=&quot;<xsl:value-of select="$parameter12"/>&quot;;
	</xsl:if>
	<xsl:if test="string-length($parameter13)>0">
		$oParameters[<xsl:value-of select="$reference13"/>]=&quot;<xsl:value-of select="$parameter13"/>&quot;;
	</xsl:if>
	<xsl:if test="string-length($parameter14)>0">
		$oParameters[<xsl:value-of select="$reference14"/>]=&quot;<xsl:value-of select="$parameter14"/>&quot;;
	</xsl:if>
	<xsl:if test="string-length($parameter15)>0">
		$oParameters[<xsl:value-of select="$reference15"/>]=&quot;<xsl:value-of select="$parameter15"/>&quot;;
	</xsl:if>
