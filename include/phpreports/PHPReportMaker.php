		...
		function createFromTemplate($sTitle="NO DEFINED TITLE",$sFile=null,$oParms=null,$oDocument=null,$oGroups=null){
			...
			if(!$oGroups){
				...
				for($i=1; $i<=$iColNum; $i++){
					...
					$sReplacedNames  .= "<COL>".ucfirst(str_replace("_"," ",$sName))."</COL>";
					$sNames			  .= "<COL TYPE=\"FIELD\">$sName</COL>";
				}

				// build the group info
				$sGroup			= "<GROUP><HEADER><ROW><REPLACE_WITH_REPLACED_COLUMN_NAMES/></ROW></HEADER><FIELDS><ROW><REPLACE_WITH_COLUMN_NAMES/></ROW></FIELDS></GROUP>";
