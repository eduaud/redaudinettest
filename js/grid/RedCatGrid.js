...
function CargaGrid(NomId)
{
	...
	//************************************ abc ***************************************
	var estilos_header=Tabla.estilos_header?Tabla.estilos_header:Tabla.getAttribute("estilos_header");
	arrayHeader=estilos_header.split("|");
	var alto_head=Tabla.estilos_header?Tabla.estilos_header:Tabla.getAttribute("alto_head");
	//********************************************************************************
 	...
 	for(var j=0;j<Tds.length;j++)
	{
		...
		multiseleccion[j] =Tds[j].multiseleccion?Tds[j].multiseleccion:Tds[j].getAttribute("multiseleccion");//MS
		...
		SumGral[j]=0;
		//opcion para poner los botones de ver,modificar y elimiar

		if(tipos[j] == 'libre')		
		{			
			cab[j]='<input type="button" value="'+valLib[j]+'" style="width:'+(largo)+'px; height:'+alto_head+'px;" class="buttonHeader" id="H'+NomId+j+'" onkeydown="TeclasEspeciales(event, this, \''+NomId+'_'+(j+1)+'_NaN\');"/>';			
		...
		else if(tipos[j] == 'binario'&&multiseleccion[j]=="S")//MS
		{
			...
			if(oclicks[j]!=null && oclicks[j]!="null" &&oclicks[j].length>0)
			{
				var funcion=oclicks[j].replace(/'/gi,"_CS_");
				funcion=funcion.replace(/"/gi,'_CD_');
				funcion='on_click="'+funcion+'"';
			}
			else
				var funcion="";
			cab[j]='<input type="button" value="'+texto+'" style="width:'+(largo)+'px; height:'+alto_head+'px;" class="'+arrayHeader[estilo_apuntador]+'" id="H'+NomId+j+'" onclick="multiselecciona('+(j+1)+', \''+NomId+'\');" onkeydown="TeclasEspeciales(event, this, \''+NomId+'_'+(j+1)+'_NaN\');" '+funcion+'/>';
			//alert('Multi - onclick="multiselecciona('+(j+1)+', \''+NomId+'\');"');
			//*****************************abc*******************************
			//incrementamos el apuntado de las clases
			estilo_apuntador++;
			//***************************************************************
		}
		else
		//colocamos las columnas diferentes a libre,oculto y multiseccion
		{
			
			cab[j]='<input type="button" value="'+texto+'" style="width:'+(largo)+'px; height:'+alto_head+'px;" class="'+arrayHeader[estilo_apuntador]+'" id="H'+NomId+j+'" onclick="ordena('+(j+1)+', \''+NomId+'\');" onkeydown="TeclasEspeciales(event, this, \''+NomId+'_'+(j+1)+'_NaN\');" />';
			//alert('No multi');
			//*****************************abc*******************************
			//incrementamos el apuntado de las clases
			estilo_apuntador++;
		...
		//insertamos los id de los elemento a array
		//ids_elementos_head='H'+NomId+j;
		//ids_elementos_head.push('H'+NomId+j); 
					
	}	
	//alert(ids_elementos_head);
	/*Insertamos la nueva Fila que contendra la cabecera*/	
	var caux="";
	caux+='<table cellpadding="'+cellP+'" cellspacing="'+cellS+'" id="Head_'+NomId+'" style="border:none">'+"\n\t"+'<tr height="'+alto_head+'" style="border:none">'+"\n";
	caux+="\t\t"+'<th align="center">'+"\n\t\t\t"+'<input type="button" value="No" style="width:25px; height:'+alto_head+'px;" class="buttonHeader">'+"\n";
	...
	{	
		...
		caux+="\n\t\t"+'<th datosdb="'+getCom[j]+'" depende="'+depen[j]+'" tipo="'+tipos[j]+'" modificable="'+mods[j]+'" mascara="'+mask[j]+'" on_click="'+oclicks[j]+'"';
		if(tipos[j] == 'libre')
		{
			var htaux=htmls[j];
	...
	if(validaElimina != 'false')
		caux+='<th align="center" width="15px" tipo="eliminador" modificable="S" id="H_'+NomId+j+'" style="border:none" height="20" valor="deleteRow"><input type="button" value="" style="width:16px; height:'+alto_head+'px;" class="buttonHeader_1"></th>';
	
	caux+='</tr></table>';
	//************************************* abc **************************************
	//caux esta cadena lleva la tabla de los encabezados del grid  
	//********************************************************************************
	...
	{	
		...
		{
            //************************************abc**************************************
            //obtenemos el estilo rallado
            if(k%2==0){
                estilo_rallado_body="";
            }else{
                estilo_rallado_body="table-duo-dark";
            }
            //*****************************************************************************
			datos+='<tr class="'+estilo_rallado_body+'" height="'+alto+'" onmouseover="selfil(this);" onmouseout="dselfil(this);" class_original="'+estilo_rallado_body+'" id="'+NomId+'_Fila'+(k-1)+'">';
			var Tds=Trs[k].getElementsByTagName('td');
			datos+='<td width="25px" class="Contador" align="center">'+k+'</td>';
			for(l=0;l<Tds.length;l++)
			{
				...
				datos+='<td t1 width="'+lons[l]+'" height="'+alto+'" ondblclick="analizaDBLCLICK(\''+dblclick[l]+'\', this)"';
				if(bloqueoDatos!="S")
				...
				if(tipos[l] == 'binario')///////////////*******************************************************************
				{
					...
					if(oclicks[l]!= null && oclicks[l]!="null" && oclicks[l].length > 0)
						var clic="analizaCLICK('"+oclicks[l]+"',this)";
					else
						var clic="";
					var extra="";
					...
					datos+='<input type="checkbox" value="SI" '+sel+' onBlur="DesEditaCelda(\''+idcelda+'\', this);" id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');"  class="check" '+extra+' onclick="'+clic+'">';
				}				
	...
	else
	{
		...
		if((paginador == 'S' || paginador == 's') && pagMetodo == 'php')
		{
			...
			{
				//INDICA POR CUAL COLUMNA SE ORDENA EL GRID DE LISTADOS
				var datosF=ajaxR(DatosFile+'&ini=0&fin='+datosxPag+'&dxp='+datosxPag+'&orderGRC='+camposbd[1]+'&sentidoOr=Asc&prefiltro=1');
				Tabla.ordenCampo=camposbd[1];
			...
			for(k=1;k<limMos;k++)
			{
               ...
               //*****************************************************************************
				...
				for(l=0;l<axF.length;l++)
				{
					...
					datos+='<td t2 width="'+lons[l]+'" height="'+alto+'" ondblclick="analizaDBLCLICK(\''+dblclick[l]+'\', this)"';
					if(bloqueoDatos!="S")
						datos+=' onclick="EditarValor(this);"';
					if(tipos[l] == 'libre')
						datos+=" onmouseover='asignaActivo(this);'";
					...
					if(tipos[l] == 'binario')//MS
					{
						...
						if(htmldb[l]=="S"&&axF[l]==-1)
							...
							datos+='<input type="checkbox" value="SI" '+sel+' id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');" valor='+valor+'  class="check" '+extra+' onclick="DesEditaCelda(\''+idcelda+'\', this);'+clic+'" >';
					}
	...
	datos+="</table>";
	//************************************* abc **************************************
	//datos lleva la cadena de las filas de datos para el grid
	//********************************************************************************
		...
		{			
			...
			{				
				...
				else if(depen[j-1] != null && depen[j-1] != "null")
				{
					if(depenMulti[j-1]=="S")
					{
						var arrDepen=depen[j-1].split("|");
						var cadLlaves="";
						var valorDpen="";
						for(var l=0;l<arrDepen.length;l++)
						{
							if(Tds[parseInt(arrDepen[l])])
								valorDepen=Tds[parseInt(arrDepen[l])+1].valor?Tds[parseInt(arrDepen[l])+1].valor:Tds[parseInt(arrDepen[l])+1].getAttribute('valor');
							else
								valorDepen="";
							cadLlaves+="&llaveaux"+l+"="+valorDepen;
						}
						caux=ajaxR(getCom[j-1]+'?iniDB=0&finDB=10000&multidependencia=1&nodependencias='+arrDepen.length+cadLlaves);
					...
					/*else
					{
						...
						caux=ajaxR(getCom[j-1]+'?iniDB=0&finDB=10000&llave='+(Tds[parseInt(depen[j-1])].valor?Tds[parseInt(depen[j-1])].valor:Tds[parseInt(depen[j-1])].getAttribute('valor')));
						var datos=caux.split('|');
						if(datos[0] == 'exito')
						{
							for(k=2;k<datos.length;k++)
	...
	//************************************* abc **************************************
	//caux esta cadena lleva la tabla del paginador en el grid  
	//********************************************************************************
	...
	//****************************abc**********************************
	//Cambiamos estilo de la tabla
     //document.getElementById('Body_listado').style.border='2px';
     //document.getElementById('Body_listado').className='prueba_tabla';
     //document.getElementById('listado').remove;
	 //document.getElementById('Hlistado1').style.backgroundImage.src="../imagenes/general/grid-buttonheader-bg.jpg";
	 //document.getElementById("listado").style.backgroundImage= "url('../imagenes/general/grid_home_pend_ic_activo.jpg')"; 
	 //Ponemos los estilos del header
	 /*for(i=0;i<estilos_header.length;i++){
		document.getElementById(ids_elementos_head[i]).className='buttonHeader_1';
	 }*/
    //*****************************************************************
...
{	
	...
	switch(tipo)
	{
		...
		case 'fecha':
			...
			if(tipo == 'entero')
				extra=extra+' onkeyup="enteros(this);" maxlength="12"';
			else if(tipo == 'decimal')
				extra=extra+' onkeyup="decimales(this);" maxlength="15"';
			else if(tipo == 'fecha' && modificable == 'S')
				extra=extra+' onfocus="calendario(this);" maxlength="10"';	
			else if(tipo == 'texto')
				extra=extra+' maxlength="255"';
			if(modificable == 'S')
		...
		if(celda.innerHTML.toLowerCase().indexOf('<select') != -1 || celda.innerHTML.toLowerCase().indexOf('<input') != -1)
			...
			//alert(depende);
			var multidependencia=Tds[fila+1].multidependencia?Tds[fila+1].multidependencia:Tds[fila+1].getAttribute('multidependencia');
			...
			{				
				...
				{
					arrDependencias=depende.split("|");
					var fff,filav,dats,val, cadLlaves="";
					for(var l=0;l<arrDependencias.length;l++)
					{
						fff=document.getElementById(tabla+'_Fila'+columna);
						filav=fff.rowIndex?fff.rowIndex:fff.getAttribute('rowIndex');
						if(filav == null)
							filav=0;
						dats=document.getElementById('Body_'+tabla);	
						Trs=dats.getElementsByTagName('tr');				
						Tds=Trs[filav].getElementsByTagName('td');
						val=Tds[parseInt(arrDependencias[l])+1].valor?Tds[parseInt(arrDependencias[l])+1].valor:Tds[parseInt(arrDependencias[l])+1].getAttribute('valor');
						cadLlaves+="&llaveaux"+l+"="+val;
					}
					var aux=ajaxR(file+'&'+'iniDB=0&finDB=10000multidependencia=1&nodependencias='+arrDependencias.length+cadLlaves);
				}
				/**************Esta parte nos configura la dependencia de combos en el grid*************************/
				else
				{
					...
					Trs=dats.getElementsByTagName('tr');				
					Tds=Trs[filav].getElementsByTagName('td');			
					//Linea Original con error var val=Tds[depende+1].valor?Tds[depende+1].valor:Tds[depende+1].getAttribute('valor');
					var val=Tds[depende].valor?Tds[depende].valor:Tds[depende].getAttribute('valor');
			...
			var tabla_pedido = document.getElementById('t').value;
			if(tabla_pedido == 'YWRfcGVkaWRvcw==' && Tabla.id == 'detallePedidosPagos' && /detallePedidosPagos_4_/.test(idcelda)){
					var listas = new Array();
					$('table#Body_detallePedidosProductos tr').each(function(index) {
							var idListas = $("#detallePedidosProductos_12_" + index).attr("valor");
							listas.push(idListas);
							});
				var urlAjax = "tiposPagosListas.php"; 
				var envio_datos = 'listas=' + listas;
				var pagos = ajaxN(urlAjax, envio_datos);
					x += pagos;
					}
					
			//Validacion de terminales bancarias de acuerdo a la sucursal
			else if(tabla_pedido == 'YWRfcGVkaWRvcw==' && Tabla.id == 'detallePedidosPagos' && /detallePedidosPagos_6_/.test(idcelda)){
				if($("#op").val() == 1){
				...
				var urlAjax = "terminalBancaria.php"; 
				var envio_datos = 'sucursal=' + sucursal;
				var bancos = ajaxN(urlAjax, envio_datos);
					x += bancos;
					}			
			/***CASO 1 DE VALIDACIONES DE COMBOS EN GRID DE GASTOS ---- AL LLENAR LOS GASTOS DE ACUERDO A LOS SUBGASTOS QUE CONINCIDAN CON LOS TIPOS DE DOCUMENTO****/
			/*else if(tabla_pedido == 'bmFfY3VlbnRhc19wb3JfcGFnYXI=' && Tabla.id == 'detalleConceptosGastosCuentasPorPagar' && /detalleConceptosGastosCuentasPorPagar_3_/.test(idcelda)){
					var id_documento = $("#id_tipo_documento_recibido").find("option:selected").val();
					var urlAjax = "gastoSubgasto.php";
					var envio_datos = 'idDocumento=' + id_documento + '&caso=1';
					var gastos = ajaxN(urlAjax, envio_datos);
					x += gastos;
					}*/
			else if(tabla_pedido == 'bmFfY3VlbnRhc19wb3JfcGFnYXI=' && Tabla.id == 'detalleConceptosGastosCuentasPorPagar' && /detalleConceptosGastosCuentasPorPagar_5_/.test(idcelda)){
					var id_documento = $("#id_tipo_documento_recibido").find("option:selected").val(); //AQUI TRAEMOS EL VALOR DEL SELECT
					var celdaGasto = idcelda.replace("5","2"); 
					var idGasto = $("#" + celdaGasto).attr("valor");
					var urlAjax = "gastoSubgasto.php";
					var envio_datos = 'idDocumento=' + id_documento + '&idGasto=' + idGasto + '&caso=2';
					var subgastos = ajaxN(urlAjax, envio_datos);
					x += subgastos;
					}
			/******************GRID DIRECCIONES DE ENTREGA EN CLIENTES***************/
			/*else if(tabla_pedido == 'bmFfY2xpZW50ZXM='){
					alert(Tabla.id);
					}*/
			
			/******************GRID DE EGRESOS CAJA CHICA***************/
			else if(tabla_pedido == 'bmFfZWdyZXNvc19jYWphX2NoaWNh' && Tabla.id == 'detalleEgresosCajaChica' && /detalleEgresosCajaChica_5_/.test(idcelda)){
					var id_documento = 1;
					var celdaGasto = idcelda.replace("5","2"); 
					var idGasto = $("#" + celdaGasto).attr("valor");
					var urlAjax = "gastoSubgasto.php";
			...
			/******************GRID DE MOVIMIENTOS DE ALMANCEN***************/
					else if(tabla_pedido == 'YWRfbW92aW1pZW50b3NfYWxtYWNlbg==' && Tabla.id == 'detalleMovimientosAlmacen' && /detalleMovimientosAlmacen_9_/.test(idcelda)){
							var almacen = $("#id_almacen").find("option:selected").val();
							var subtipo = $("#id_subtipo_movimiento").find("option:selected").val();
							var celdaProd = idcelda.replace("9","2"); 
							var idProd = $("#" + celdaProd).attr("valor");
							
							var urlAjax = "obtenLoteExistencia.php";
							var envio_datos = 'almacen=' + almacen + '&subtipo=' + subtipo + '&producto=' + idProd;
							var loteExistencia = ajaxN(urlAjax, envio_datos);
							/*var respuesta = loteExistencia.split(":");
							
							var celdaExistencia = idcelda.replace("7","8"); 
							$("#" + celdaExistencia).attr("valor", respuesta[1]);
							$("#" + celdaExistencia).html(respuesta[1]);
			...
			/*
			 *
			 *	LFLORES	::	FILTRADO DE PRODUCTOS -> ORDEN DE RECOLECCION
			 *
			 */
		 
			else if(tabla_pedido == 'bmFfb3JkZW5lc19yZWNvbGVjY2lvbl9jbGllbnRlcw==' 
				 && Tabla.id == 'detalleOrdenesRecoleccionClientesProductos' 
				 && /detalleOrdenesRecoleccionClientesProductos_3_/.test(idcelda))
			{
				var id_documento = $("#id_pedido").find("option:selected").val();
				if (id_documento == 0)
				{
					alert('Favor de seleccionar un pedido valido'); 
					return false;
				}
				else
				{
					var urlAjax = "llenaDato.php";
					var envio_datos = 'id=' + id_documento + '&dato=productosPedidoCliente';
					var productos = ajaxN(urlAjax, envio_datos);
					x += productos;						
				}				
			}			
			else
		...
		case 'buscador':
			...
			if(modificable == 'S')
				aux='<input extra="0"; id="c'+idcelda+'" type="text" style="width:'+(largo-1)+'px; height:'+(alto-1)+'px;" class="celdaTexto" value="'+valor+'" onblur="DesEditaCelda(\''+idcelda+'\', this);" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');" onkeyup="Rebusca(event, \''+idcelda+'\', this)" onmouseover="this.extra=1;" onmouseout="this.extra=0;" onkeypress=""/>';
			else	
...
case 'buscadorC':	//agregado tipo de campo para Registro de Cobros CI 13/06/12					
			var valor=celda.innerHTML;
			var aux="";
			var largo=celda.offsetWidth;
			var alto=celda.offsetHeight;
			...
			var alto=celda.offsetHeight-2;
			if(modificable == 'S')
				aux='<input extra="0"; id="c'+idcelda+'" type="text" style="width:'+(largo-1)+'px; height:'+(alto-1)+'px;" class="celdaTexto" value="'+valor+'" onblur="DesEditaCelda(\''+idcelda+'\', this);" onkeydown="TeclasEspecialesC(event, this, \''+idcelda+'\');" onkeyup="activaBuscadorC(event, \''+idcelda+'\', this)" onmouseover="this.extra=1;" onmouseout="this.extra=0;"/>';
			else	
				aux='<input id="c'+idcelda+'" type="text" style="width:'+largo+'px; height:'+alto+'px;" class="celdaTextoD" onkeydown="TeclasEspecialesC(event, this, \''+idcelda+'\');" value="'+valor+'" onblur="DesEditaCelda(\''+idcelda+'\', this);" readonly/>';				
...
{	
	...
	var funclick=cab.on_click?cab.on_click:cab.getAttribute('on_click');	
	var modificable=cab.modificable?cab.modificable:cab.getAttribute('modificable');
	...
	switch(tipo)
	{
		...
		case 'fecha':			
			...
			if(tipo == 'decimal' && obj.value.substring(0,1) == '.')
			{
				celda.valor='0'+obj.value;			
				celda.setAttribute("valor",'0'+obj.value);			
			}
			else
			{
		...
		case 'buscador':	
			...
			if(celda.valor != obj.value)
				nver=true;
			var valoract=celda.valor?celda.valor:celda.getAttribute("valor");
			if(valida != null && valida != "null" && valida.length > 0)
			{
				var valTemp=valida.replace('#',yreal);
				valTemp=valTemp.replace('$DATO', obj.value);
			...
			var stringConsulta 	= celda.valor.split(":");
			var stringDato 		= stringConsulta[1];
			var stringId			= stringConsulta[0];
			
			if(obj.value.length == 0)
				celda.innerHTML='&nbsp;';	
			else
				celda.innerHTML=stringDato;
				
			break;
			
		case 'buscadorC':			
			var aux=document.getElementById(tabla);
			//alert('Desedita '+aux.getAttribute('elefoc'));
			var aux=aux.elefoc?aux.elefoc:aux.getAttribute('elefoc');						
			if(aux == 'com'+cad)
				return false;			
			if(celda.valor != obj.value)
	...
	if(funclick != null && funclick != 'null' && funclick.length > 0 && tipo== "binario")
	{
		var funaux=funclick;
		//alert(funaux);
...
function TeclasEspeciales(eve, obj, cad)
{
	...
	{ 
		...
		if(tipoSig == 'libre' || tipoSig == 'eliminador' || tipoSig == 'oculto' || modsig == 'N' || modsig == 'n')
		{
			...
			{	
				...
				/*Agregado para manejo de foco en Notas de Venta*/
				if(tabla == 'detallenotasdeventa' || tabla == 'foliosdisponibles'){  //add para que seleccione el texto 
				   if(ax){
				   if(typeof document.selection != 'undefined' && document.selection){        //método IE 
						...
						ax.value='';  
						ax.focus(); 
						var str = document.selection.createRange().tex;  
						ax.value=tex; 
						str.move("character", ax.value.length);  
						str.moveEnd("character", 0);  
						str.select(); 
					} 
					else if(typeof ax.selectionStart != 'undefined'){                    //método estándar 
				   
						ax.focus();
						ax.setSelectionRange(0,ax.value.length);  
		...
		{			
			...
			EditarValor(ncelda);			
			ax=document.getElementById('c'+real);
			if(tabla == 'detallenotasdeventa' || tabla == 'foliosdisponibles'){
			   if(ax){
				    if(typeof document.selection != 'undefined' && document.selection){        //método IE 
						var tex=ax.value; 
						ax.value='';  
	...
	//else if(key == 40 || (key >= 65 && key <= 90 && $(obj).val().length >= 4))
	else if(key == 40)
	{	
		...
		if(tipo == 'buscador')	
		{
			...
			ejemplox="prabando blur";//onclick="ocultaBus(this.value, \''+cad+'\');"
		...
		//Variables que concatenaran parametros de busqueda adicionales dentro de grid detalle en el combo buscador
		var proveedor = "";
		var sucursal_sel = "";
		//Si la tabla es ordenes de compra
		if(document.getElementById('t').value == 'YWRfb3JkZW5lc19jb21wcmFfcHJvZHVjdG9z'){
				//Buscamos el proveedor seleccionado
				var prov_sel = $("#id_proveedor").find("option:selected").val();
				//Armamos el envio de la variable por get al archivo getCombo.php
				proveedor += "&proveedor=" + prov_sel;
					
				}
		//Si la tabla es solicitud de devolucion a cedis
		else if(document.getElementById('t').value == 'bmFfc29saWNpdHVkX2Rldm9sdWNpb25fY2VkaXM='){
				//Buscamos la sucursal seleccionada
				var sucursalS = $("#id_sucursal").find("option:selected").val();
				//Armamos el envio de la variable por get al archivo getCombo.php
				sucursal_sel += "&sucursal_sel=" + sucursalS;
				}
					
			//Concatenamos las variables con los parametros adicionales
			var ax=ajaxR(rfile+'&llave='+obj.value + proveedor + sucursal_sel);
			var ax=ax.split('|');
			if(ax[0] == 'exito')
			{
...
function AplicaFormula(obj)
{
	...
	var cols=filas[0].getElementsByTagName('th');
...
{	
   ...
   if((tabla == 'detalleArticulos' || tabla == 'detalleArticulosBasicos' || tabla == 'detalleArticulosEspeciales' || tabla =='detalleArticulosProduccion' || tabla == 'detalleArticulosCompra') && ($('#fecha_evento').val() == "" || $('#fecha_entrega_articulos').val() == "" || $('#fecha_recoleccion').val() == "" || $("#hora_entrega").val() == '0' || $("#hora_entrega2").val() == '0' || $("#hora_recoleccion").val() == '0' || $("#hora_recoleccion2").val() == '0' || $("#id_tipo_evento_localizacion").val() == '0' || $("#id_tipo_documento").val() == '0'))
	{
		alert("Los campos relacionados a las fechas de entrega, evento y recolección no pueden estar vacíos");
		return false;
	...
	var newRow=Tabla.insertRow(-1);
	newRow.className="table-duo";
	newRow.onmouseover=function(){selfil(this);}
	newRow.onmouseout=function(){dselfil(this);}
	newRow.id=tabla+'_Fila'+newFila;	
	document.getElementById(tabla+'_Fila'+newFila).setAttribute('class_original','table-duo'); 
	...
	for(j=1;j<lon;j++)
	{
		...
		{	
			...
			newCell.style="border-left: none;";
		}
		if(largo >= 0)
		{
			newCell.width=largo;
  ...
  /*Excepcion para casa Ibarra en Alta de Productos*/
	if(tabla == 'detalleproducto'){
	   CambiaPresentacion('detalleproducto',newFila); 	
	}
	
  /*Excepcion para registro de Cobros*/
    if(tabla == 'regCobros'){
		AgregaValorCli('regCobros',newFila); 
	}
		
	}
	
		
	if(tabla=='detalleArticulos'){
		llenarPorcentaje('detalleArticulos',9,$("#porcentaje").val());
		var i=NumFilas('detalleArticulos')-1;
		aplicaValorXY(tabla,10,i,$('#factor_costo_total_producto').val());
		valorXY(tabla,10,i,$('#factor_costo_total_producto').val());	
    }
	if(tabla=='detalleArticulosBasicos'){
		llenarPorcentaje('detalleArticulosBasicos',11,$("#porcentaje").val());
		llenarFactorCosto('detalleArticulosBasicos',12)
    }
	if(tabla=='detalleArticulosEspeciales'){
		llenarPorcentaje('detalleArticulosEspeciales',12,$("#porcentaje").val());
		llenarFactorCosto('detalleArticulosEspeciales',13)
    }
	if(tabla=='detalleArticulosProduccion'){
		llenarPorcentaje('detalleArticulosProduccion',6,$("#porcentaje").val());
		llenarFactorCosto('detalleArticulosProduccion',7)
    }
	if(tabla=='detalleArticulosCompra'){
		llenarPorcentaje('detalleArticulosCompra',6,$("#porcentaje").val());
...
function llenarPorcentaje(grid,numCampo,porcentaje)
{
	...
	{
		 // idTabla='#'+grid+'_'+numCampo+'_'+i;
		  //$(idTabla).text(porcentaje);
...
//Funcion que llena los campos  con el valor  de factor costo
function llenarFactorCosto(grid,numCampo)
{
	var costo=$('#factor_costo_total_producto').val();
	for(i=0;i<=NumFilas(grid);i++)
	{
		
		   var idTabla='#'+grid+'_'+numCampo+'_'+i;
		  //$(idTabla).text($('#factor_costo_total_producto').val());
		  //valorXY();
		  aplicaValorXY(grid,numCampo,i,costo);
		  $(idTabla).text($('#factor_costo_total_producto').val());
	}	
...
function EliminaFila(cad)
{
	...
	//alert(cad+ ' / '+tabla+'_0_'+fila);
	//*******************abc****************************
	//***************exepciones para eliminar***********
		
	if(tabla=="serviciosGrid")
	{
		var id_auxDetalle=document.getElementById(tabla+'_0_'+fila).getAttribute("valor");
		
		//alert(id_auxDetalle);
		
		if(id_auxDetalle != 'undefined' && id_auxDetalle != '0' && id_auxDetalle != 'NO')
		{
				if(!confirm("¿Desea eliminar el descuento del servicio en lista de precios?"))
				{
					return false;
				}
				else
				{
					//eliminamos este registro de la tabla 
									
					var auxElim=ajaxR("../ajax/especiales/eliminaIdTabla.php?opcion=1&id="+id_auxDetalle);
					var aElim=auxElim.split("|");
					if(aElim[0] != 'exito')
					{
	...
	//Verificamos si se puede elminar el producto del detalle de pedido
	else if(tabla == "detallePedidosProductos" && $("#op").val() == 2){
			var id_pedido = $("#" + tabla + "_1_" + fila).attr("valor");
			var ruta = "eliminaProductoPedido.php";
			var envio = "id=" + id_pedido;
			var respuesta = ajaxN(ruta, envio);
			if(respuesta > 0){
					alert("No se pueden modificar los productos del pedido\ndebido a que el pedido ya esta relacionado a un movimiento de almacen");
					return false;
					}
			
			}
	
	else if(/*tabla=="detalleArticulos" ||*/ tabla=="detalleOCOperadoraFranquicia"){
		//creamos las instancias a los objetos
	   ...
	   if(!objTotal)
		   return false;
	   //obtenemos los elemnetos que se van a eliminar   
	   
	   subtotal_eliminado=parseFloat((celdaValorXY(tabla,14,fila)=="")?0:celdaValorXY(tabla,14,fila));
	   //eliminamos las comas de las cantidades en los textbox
	   strSubtotal=objSubtotal.value.replace(/[,]/g, '');
	   subtotal=parseFloat(strSubtotal)-subtotal_eliminado;
	  
	   //actualizamos los nuevos datos
	   objSubtotal.value=Mascara("###,###.##",redondear(subtotal,2));
	   //falta agregar los descuentos
	   objTotal.value=Mascara("###,###.##",redondear(subtotal,2));
	    
	}	
	
	else if(tabla=="detalleCuentasPorPagarCosteo"){
			var idElimina = $("#detalleCuentasPorPagarCosteo_1_" + fila).attr("valor");
			var url = "eliminaCXP.php";
			var envia_datos = "id=" + idElimina;
			ajaxN(url, envia_datos);
			}
	...
	var hTds=hTrs[0].getElementsByTagName('th');
...
Si se elimina una fila del grid detalle de nostas de venta se Actualiza el SUBTOTAL de encabezado
*/	
     if(tabla == 'detallenotasdeventa'){
	     actualizaImportes('detallenotasdeventa',filaB);	 
	 }
	...
	//INICIA BLOQUE DE GRIDS CORRESPONDIENTE A SOLICITUD DE COTIZACION
	if(tabla == "detalleArticulos" || tabla == "detalleArticulosBasicos" || tabla == "detalleArticulosEspeciales" || tabla == "detalleArticulosProduccion" || tabla == "detalleArticulosCompra"){
		actualizaImportes("todoSolCotizacion", 0);
	}	
	//TERMINA BLOQUE DE GRIDS CORRESPONDIENTE A SOLICITUD DE COTIZACION	
	
	if(tabla=="detalleNotasProducto"){
		//llenaCamposNotaProductos(fila);
	...
	}else if(tabla=="detalleNotasServicios"){
		sumaGridsNotasServicios();
	}
	else if(tabla=="detallePedidosProductos"){
		calculaTotal();
		//calculaSaldo()
		}
	else if(tabla=="detallePedidosFletes"){
		montoTotalFletes();
		calculaSaldo();
		}
	else if(tabla=="detallePedidosPagos"){
		calculaTotalPagos();
		}
	else if(tabla=="detalleProductosCuentasPorPagar"){
			calculaTotalesN('detalleProductosCuentasPorPagar', 6, 'subtotal_productos');
			calculaSubtotalesN();
			}
	else if(tabla=="detalleConceptosGastosCuentasPorPagar"){
			calculaTotalesN('detalleConceptosGastosCuentasPorPagar', 9, 'subtotal_gastos');
			calculaSubtotalesN();
			}
	else if(tabla=="detallePagosCuentasPorPagar"){
			calculaTotalesN('detallePagosCuentasPorPagar', 6, 'subtotal');
			calculaSubtotalesN();
			}
	else if(tabla=="detalleCosteoProductos"){
	...
	else if(tabla=="detallePagosCuentasPorCobrar"){
			calculaTotalesN('detallePagosCuentasPorCobrar', 8, 'subtotal'); 
			saldoCXC();
			}
	else if(tabla=="detalleProductosOrdenesCompra"){
			calculaTotalesN('detalleProductosOrdenesCompra', 8, 'subtotal'); 
			ivaTotalOrdenesCompra();
			}
			
			
			
	/****************Modificacion para reconstruir los ID del Grid despues de un borrado de celda*******************/
	var cuentaFilasGrid = $('table#Body_' + tabla + ' tr').length; //Obtenemos el numero de filas en el grid para validaciones posteriores
	
	$('table#Body_' + tabla + ' tr').each(function(index) {   //Recorremos 
			$(this).attr("id", tabla + "_Fila" + index);  // Cambiamos el ID de la fila actual en el recorrido 
	
			var cuentaColumnasGrid = $(this).children('td:not(.Contador)').length - 1;  //Contamos el numero de columnas quitando el contador
			$(this).children('td:not(.Contador)').each(function(index2) {  //Recorremos el numero de columnas excepto el contador
					$(this).attr('id', tabla + '_' + index2 + '_' + index);  //Cambiamos el ID de la columna actual del recorrido
					
					if(index2 == cuentaColumnasGrid){  //En la ultima vuelta del recorrido cambiaremos las propiedades del eliminador de filas
							$(this).children().attr('id', tabla + '_' + index2 + '_' + index); //Cambiamos el ID de la imagen en la celda
							$(this).children().attr('onclick', 'EliminaFila("' + tabla + '_' + index2 + '_' + index + '")'); //Cambiamos el parametro de la funcion
							}
					});
...
{
	//alert("dselfil");
	class_original=obj.getAttribute("class_original");
	//alert(class_original);
	if(obj.className != 'actCell')
	      obj.className=class_original;
		//obj.className='NormalCell';
}
...
function teclasEspBus(eve, cad, obj)
{
	...
	if(key == 13){
	   if(tabla == 'regCobros'){
	       ...
	       ocultaBusC(obj.value, cad, obj);
		   //setTimeout(ocultaBusC(obj.value, cad, obj), 5);
	   }else
...
function RecargaGrid(tabla, filar)
{
	...
	for(var i=1;i<Tds.length;i++)
	{
		...
		oclick[i-1]=Tds[i].on_click?Tds[i].on_click:Tds[i].getAttribute('on_click');
	...
	if(arrF[0] == 'exito')
	{
		...
		for(k=inicont;k<limMos;k++)
		{
            ...
            //*****************************************************************************
			var axF=arrF[k].split('~');
			NewTabla+='<tr class="'+estilo_rallado_body+'" height="'+alto+'" onmouseover="selfil(this);" onmouseout="dselfil(this);" class_original="'+estilo_rallado_body+'" id="'+tabla+'_Fila'+((k+indk)-1)+'">';
			NewTabla+='<td width="25px" class="Contador" align="center">'+(k+indk)+'</td>';
			for(l=0;l<axF.length;l++)
			{
				...
				NewTabla+='<td t3 width="'+lons[l]+'" height="'+alto+'" ondblclick="analizaDBLCLICK(\''+dblclick[l]+'\', this)"';
				...
				if(tipos[l] == 'binario')//MS
				{
					...
					if(oclick[l]!= null && oclick[l]!="null" && oclick[l].length > 0)
						var clic="analizaCLICK('"+oclick[l]+"',this)";
					else
						var clic="";
					...
					NewTabla+='<input type="checkbox" value="SI" '+sel+' onBlur="DesEditaCelda(\''+idcelda+'\', this);" id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');" valor='+valor+'  class="check" '+extra+' onclick="'+clic+'">';
				}////////=***********************************************************************
				else if(tipos[l] == 'libre')//MS
				{							
					if(htmldb[l]!="S")
	...
	/***Esta instruccion if se comento a partir de un error que genera al recargar grids con combos ******/
	/*if(cdg > 0)
	{
		for(i=0;i<NumFilas(tabla);i++)
		{
			for(j=0;j<cdg;j++)
			{
				...
				if(multidependencia=="S")
				{
					var arrDependencias=vdep.split("|");
					var cadDep="";
					for(var k=0;k<arrDependencias.length;k++)
					{
						vdep=parseInt(arrDependencias[k]);
						var val = Tds[vdep+1].valor?Tds[vdep+1].valor:Tds[vdep+1].getAttribute('valor');
						cadDep+="&llaveaux"+k+"="+val
					}
					caux=ajaxR(vq+'?iniDB=0&finDB=10000&multidependencias=1&nodependencias='+arrDependencias.length+cadDep);
				}
				else
				{	
					vdep=parseInt(vdep);
					...
					case 'fecha':						
						...
						{					
							if(valor > 0 && valor != " "){
								...
								celda.innerHTML=Mascara(maskara,0);
							}
							return true;
						}			
						if(valorAnt != valor)
...
{	
	...
	for(var i=0;i<numdatos;i++)
	{
		...
		for(var j=1;j<Tds.length;j++)
		{
			...
			{   
				 var TipoAux = tipos[j-1].toUpperCase();
				 if(TipoAux.indexOf('FECHA')!=-1) {
					 //Si el nombre de Cabecera de Grid contiene Fecha Buscamos Formato Fecha
				   
					var Strax=Tds[j].valor?Tds[j].valor:Tds[j].getAttribute('valor');
					
					for(var k=0; k<Strax.length; k++){
                        var caracter = Strax[k];
					    
					   if(k==4){
						 if(caracter == '-' || caracter == '/'){
					        //el formato q tiene es YYYY-mm-dd o YYYY/mm/dd
					    	//solo reemplazamos el caracter / por -
						 if(Strax.indexOf('/')!=-1)	
							ax = Strax.replace("/","-");
						 else
						    ax = Strax;
					   ...
					   if(k==2){
						   if(caracter == '-' || caracter == '/'){
						      //tiene formato dd-mm-YYYY o dd/mm/YYYY
							  //cambiamos formato
							 if(Strax.indexOf('/')!=-1)	 
							    var getFecha = Strax.split("/");
								
							 if(Strax.indexOf('-')!=-1)	 
							    var getFecha = Strax.split("-");
								
								ax = getFecha[2]+'-'+getFecha[1]+'-'+getFecha[0];
						   }
					  }
...
function analizaCLICK(func,obj)
{
	var funcion=func.replace(/_CS_/gi,"'");	
	funcion=funcion.replace(/_CD_/gi,'"');	
	func=funcion;
	var aux=obj.id.split("_");
	var tabla=aux[0];
	var fila=parseInt(aux[1]);
...
function borraContCeldaXY(grid,x,y)
{
	objCelda=document.getElementById(grid+"_"+x+"_"+y);
	if(!objCelda)
		return false;
	objHeader=document.getElementById("H_"+grid+x);
	if(!objHeader)
		return false;
	tipo=(objHeader.tipo)?objHeader.tipo:objHeader.getAttribute("tipo");
	if(tipo=="checkbox")
	{
		objCampo=document.getElementById(grid+"_"+x+"_"+y);
		if(!objCampo)
			return false;
...
/*Agregado para Registro de Cobron CI*/
function TeclasEspecialesC(eve, obj, cad){
	...
	{			
		...
		//alert('Enter '+  tipoSig );
		if(tipoSig == 'libre' || tipoSig == 'eliminador' || tipoSig == 'oculto' || modsig == 'N' || modsig == 'n' || tipoSig == 'buscadorC')
		{		
			var ncol=NumColumnas(tabla);
			//alert(columna+' -> '+ncol);
			for(lk=(columna+1);lk<ncol;lk++)
			...
			if(ncelda)
			{	
				DesEditaCelda(cad, obj);	//cambiar a DesEditaCeldaC			
				EditarValor(ncelda);		//cambiar a EditarValorC	
				ax=document.getElementById('c'+real);
				/*Agregado para manejo de foco en Notas de Venta*/
				if(tabla == 'detallenotasdeventa' || tabla == 'foliosdisponibles'){  //add para que seleccione el texto 
				   if(ax){
		...
		{			
			//alert('ok1'+cad+'-'+obj);
			DesEditaCelda(cad, obj);  //cambiar a DesEditaCeldaC
			EditarValor(ncelda);	//cambiar  a 	EditarValor
			//alert('real: '+real);
			ax=document.getElementById('c'+real);
			if(tabla == 'detallenotasdeventa' || tabla == 'foliosdisponibles'){
			   if(ax){
	...
	{			
		...
		{			
			...
			aux+='<select size="4" style="width:'+(largo+3)+'px; height:100px" id="com'+cad+'" onclick="ocultaBusC(this.value, \''+cad+'\',this);" onkeydown="teclasEspBus(event ,\'com'+cad+'\', this);"';
			aux+='onfocus="document.getElementById(\''+tabla+'\').elefoc=\'com'+cad+'\';document.getElementById(\''+tabla+'\').setAttribute(\'elefoc\', \'com'+cad+'\');"';
			aux+=' onblur="OcultaBusSC(\''+cad+'\');" class="combos">';			
			var valaux=obj.value;
			var rfile=datosdb;
			...
			//alert('paso'+obj.value.length);
			if(obj.value.length == 0){//si la celda esta vacio hace la busqueda 
			...
			if(ax[0] == 'exito')
			{
				for(i=2;i<ax.length;i++){
				    var v_ax = ax[i].split(':');
				    aux+='<option value="'+v_ax[1]+'">'+v_ax[0]+'</option>';
				}
			}
			else
				alert("No se pudo cargar datos desde "+rfile+'&llave='+obj.value+"\n\n"+ax);
			...
			celda.innerHTML=aux;
			if(tabla == 'regCobros'){
			   document.getElementById('bandeEdoCli').value=1;
			}
			setTimeout("document.getElementById('com"+cad+"').focus();", 100);
...
}//fin function TeclasEspecialesC

function ocultaBusC(val, id, objSel){
	//alert('valor : '+objSel.value+'-'+objSel.options[objSel.selectedIndex].text);
	...
	obj.valor=objSel.options[objSel.selectedIndex].text;
	obj.setAttribute('valor', objSel.options[objSel.selectedIndex].text);
	
	if(document.getElementById('auxCliente'))
	   document.getElementById('auxCliente').value = objSel.value;
   ...
   //alert('fin ocultaBusC: '+id);
	EditarValor(document.getElementById(id));	
	obj=document.getElementById('c'+id);
	//obj.innerHTML ='que pasa';
	//obj.focus();
	}
	
	
}//fin function oculta busc


function activaBuscadorC(evento, cad, obj){

   if((evento.keyCode==40&obj.value.length>=3)||obj.value.length>=3)
	{ //entra aqui buscador
		 RebuscaC(evento, cad, obj);
	}
	
}
function RebuscaC(eve, cad, obj){
	...
	{	
		...
		if(document.getElementById('com'+cad)){
		   	ocultaBus(obj.value,cad);
		}
		...
		//Linea 5656
		if(obj.value.length >= 3){//si la celda esta vacio hace la busqueda 
		//Variables que concatenaran parametros de busqueda adicionales dentro de grid detalle en el combo buscador
		var proveedor = "";
		var sucursal_sel = "";
		//Si la tabla es ordenes de compra
