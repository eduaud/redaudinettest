...
function CargaGrid(NomId)
{
	...
	if(validaElimina != 'false')
		caux+='<td align="center" width="15px" tipo="eliminador" modificable="S" id="H_'+NomId+j+'" style="border:none" height="20" valor="deleteRow"><input type="button" value="" style="width:16px; height:'+alto_head+'px;" class="buttonHeader_1"></td>';
	...
	{	
		for(k=1;k<Trs.length;k++)
		{
            ...
            if(k%2==0){
                estilo_rallado_body="NormalCell";
            }else{
                estilo_rallado_body="NormalCell_gris";
            }
            //*****************************************************************************
		...
		if(celda.innerHTML.toLowerCase().indexOf('<select') != -1 || celda.innerHTML.toLowerCase().indexOf('<input') != -1)
			...
			{				
				...
				else
				{
					...
					//alert(depende+'-'+Tds[depende+1].valor);
...
{	
	...
	document.getElementById(tabla+'_Fila'+newFila).setAttribute('class_original','NormalCell'); 
...
function EliminaFila(cad)
{
	...
	//alert(tabla+" - "+columna+" - "+fila);
	//*******************abc****************************
	//***************exepciones para eliminar***********
	if(tabla=="detallepedidos" || tabla=="detalleOCOperadoraFranquicia"){
	   ...
	   monto_iva_eliminado=parseFloat((celdaValorXY(tabla,9,fila)=="")?0:celdaValorXY(tabla,9,fila));
	   subtotal_eliminado=parseFloat((celdaValorXY(tabla,6,fila)=="")?0:celdaValorXY(tabla,6,fila));
	   ...
	   strIVA=objIVA.value.replace(/[,]/g, '');
	   iva=parseFloat(strIVA)-monto_iva_eliminado;
	   //actualizamos los nuevos datos
	   objSubtotal.value=Mascara("###,###.##",redondear(subtotal,2));
	   objIVA.value=Mascara("###,###.##",redondear(iva,2));
	   objTotal.value=Mascara("###,###.##",redondear(subtotal+iva,2));
...
*/	
     //excepciones de eliminado de cuentas
	 if(tabla == 'detalleCuentasPagarGastosManual'){
	     actualizaImportes('detalleCuentasPagarGastosManual',0);	 
	 }
	 else if(tabla == 'detalleCxcOperadora'){
	     actualizaSaldoPendienteCxC();	 
	 }
	 else if(tabla == 'detalleCuentasPagarPagos'){
	     actualizaSaldoPendienteCxP();	 
	 }
	 //-----------------------------------------------------------------
