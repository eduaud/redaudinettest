$(document).ready(function(){
	...
	$("#buscarDatos").click(function(){
		...
		$.getJSON("almacen_entradas.php?idPantalla=13&accion=1&tipoDoc="+idTipoDoc+"&idEntrada=" + idDoc, function(data){ 
			...
			$("#tipoDocumento").val(data[2]);
			
			$("#infoEntrada").html(data[3][0][1] + "" + data[3][0][2]);
...
function abrirThickBox(id, valoresExtras){
	...
	ruta="almacen_entradas.php?accion=2&datos="+id+"&val1="+val1+"&val2="+val2+valoresExtras+"&tipoDocumento="+$("#tipoDocumento").val()+"&height=500&width=600&modal=false";//"agendaSpa.php?accion=2&idCliente=" + idCliente + "&height=570&width=1000&modal=true";
...
function btnMod(id){
	...
	valores = id.split("_");
	abrirThickBox(idPadre, "&valCantidad="+$("#fila_"+id+"_2").html()+"&valLote="+$("#fila_"+id+"_3").html()+"&valUbicacion="+valores[0]+"&valDoc="+$("#fila_"+id+"_4").html()+"&clasifLlanta="+valores[1]+"&fila="+id);
}
...
function guardarCambios(){
	...
	$.getJSON("almacen_entradas.php?idPantalla=13&accion=3&datos=" + cadena, function(data){ 
		valores = data.split("|");
		alert(valores[1]);
		if(valores[0] == 'ok'){
			console.log(data);
			location.href="almacen_entradas.php";
		}
	});
