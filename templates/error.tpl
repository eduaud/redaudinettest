...
<input type="hidden" name="campoTexto" value="">
	...
	{if (($reporte ge 1 and $reporte le 78)  or $reporte eq 104  ) }
		
			
		<tr>
			<td colspan="4" class="tdContorno">
            	{if $reporte eq 16 or $reporte eq 13}
                    &nbsp;&nbsp;<span class="requerido">Mes:</span>&nbsp;&nbsp;
                    <select name="mes" class="combos"  id="mes">
                    ...
                    &nbsp;&nbsp;<span class="requerido">A&ntilde;o:</span>&nbsp;&nbsp;
                    <select name="anio" class="combos"  id="anio">
                    {html_options values=$anios output=$anios}
                    </select>
                {/if}
                
                {if $reporte eq 16 or $reporte eq 13 or $reporte eq 25 or $reporte eq 26 or $reporte eq 11 or $reporte eq 12 or $reporte eq 8 or $reporte eq 5 or $reporte eq 10 or $reporte eq 3 or $reporte eq 2 or $reporte eq 1 or $reporte eq 7 or $reporte eq 6 or $reporte eq 14 or $reporte eq 15 or $reporte eq 4 or $reporte eq 20 or $reporte eq 21 or $reporte eq 22}
                    &nbsp;&nbsp;<span class="requerido">Sucursal:</span>&nbsp;&nbsp;
                    <select name="sucursalC" class="combos"  id="sucursalC">
                    {html_options values=$sucs_val output=$sucs_text}	
                    </select>                    
                {/if}            	
                
                
				{if $reporte eq "99999"}
					&nbsp;&nbsp;<span class="requerido">Fecha de corte:</span>&nbsp;&nbsp;
					<input type="text" name="fechaCorte" id="fechaCorte" maxlength="10" size="12" class="campos" onFocus="calendario(this);" value="{$smarty.now|date_format:"%Y-%m-%d"}"/>
					
					

				{/if}
				{if $reporte eq "1" or $reporte eq "2"}
					&nbsp;&nbsp;<span class="requerido">Versi&oacute;n del reporte:</span>&nbsp;&nbsp;
					<select class="combos" name="version"  >
						<option value="d">Detallado</option>
						<option value="n">Simple</option>
					</select>
				{/if}
                
				{if $reporte eq "1" or $reporte eq "2" or $reporte eq "6" or $reporte eq "7" or $reporte eq "11" or $reporte eq "12" or $reporte eq "10" or $reporte eq "20"  or $reporte eq "24"  or $reporte eq "25" or $reporte eq "26"  or $reporte eq "5" or $reporte eq "3" or $reporte eq "14" or $reporte eq "4" or $reporte eq "21" or $reporte eq "22"  or $reporte eq "15"}
                	<span class="requerido">Rango de fechas
					{if $reporte eq "2" or $reporte eq "3" or $reporte eq "7" or $reporte eq "11"}
						 de la factura
					{elseif $reporte eq "6"}
						de la Nota de Cr&eacute;dito 
					{/if}					
					:</span>					
					&nbsp;&nbsp;del&nbsp;
	    			<input type="text" name="fecha1" id="fecha1" maxlength="10" size="12" class="campos" onFocus="calendario(this);" value="{$smarty.now|date_format:"%Y-%m-%d"}" />&nbsp;al&nbsp;
					<input type="text" name="fecha2" id="fecha2" maxlength="10" size="12" class="campos" onFocus="calendario(this);" value="{$smarty.now|date_format:"%Y-%m-%d"}"/>								
				{/if}
			</td>
		</tr>
		<tr valign="top">
			<td class="subtitulo">TABLA:<br />
				<select name="tabla" class="combos" onchange="cambiaCampos(this)" id="tabla">
					<option value="0">-- Seleccionar --</option>
					{html_options options=$arrayTablas}
				</select>
		  </td>
			<td class="subtitulo">CAMPO:<br />
				<select name="campo" id="campo" class="combos" size="14" >
					<option value="0">-- Seleccionar --</option>
				</select>
			</td>
			<td class="subtitulo">CRITERIO:<br />
			...
			<td class="subtitulo" valign="top" >VALOR:<br />
		...
		<tr>
			<td class="subtitulo">INCLUSI&Oacute;N:<br />
				<input type="radio" name="limit" value="AND" id="limit" checked="checked"/>Y <input type="radio" name="limit" value="OR" id="limit"/>O
...
function cambiaCampos(combo){
	if(combo.name!='ejercicios')
		if(combo.name=='nivelSuperior'){
			if(document.getElementById("tipos").value=='1')
				return true;
			makeRequest('../ajax/camposTablas.php?tabla='+combo.value+'&opcion=conta');
		}else
			makeRequest('../ajax/camposTablas.php?tabla='+combo.value);
	else
		makeRequest('../ajax/camposTablas.php?tabla='+combo.name+'&ejercicio='+combo.value+'&opcion=conta');
}
...
function cambiaCombo(){
	...
	if(http_request.readyState == 4){
		...
		{
			alert('Hubo problemas con la petici&oacute;n.');
		}
	}
}
...
function agregaCriterio(objform){
	...
	if(valor=='' && criterio<12){
		alert("Especif&iacute;que un valor.");
		return false;
	}
	switch(criterio){
		case '0':
...
function generaReporte(opcion,combo,idRep)
{
	...
	if(document.forms.formax.mes)	
		opcionales+=(document.forms.formax.mes.value+"|");
	if(document.forms.formax.anio)	
		opcionales+=(document.forms.formax.anio.value+"|");	
	if(document.forms.formax.sucursalC)	
		opcionales+=(document.forms.formax.sucursalC.value+"|");	
	...
	window.open("procesaReportes.php?parametros="+cadena+"&opcion="+opcion+"&idRep="+idRep+"&opcionales="+opcionales+"&titulo="+document.forms.formax.titulo.value+"&opPagoRegistro="+opPagoRegistro+orden, "_blank", especificaciones);
