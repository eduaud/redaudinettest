...
GROUP BY a.id_cliente";    
	...
	if($num >0){
	   ...
	   $horaActMexico = date("Y-m-j H:i:s"); //enves de NOW() colocamos la hora de mexico y no del servidor.
	  
	   $strSQLFecha="SELECT folio_nc_facturas_recibos_honorarios FROM sys_sucursales where id_sucursal='".$_SESSION["USR"]->sucursalid."'";
	   $respuestaFecha=mysql_query($strSQLFecha) or die("Error at rowsel $strSQLFecha :: ".mysql_error());
	   $rowFecha=mysql_fetch_row($respuestaFecha);
	   mysql_free_result($respuestaFecha);
	   $diasMenos=$rowFecha[0];	
	   	   
	   $newdate = strtotime ( '-'.$diasMenos.' day' , strtotime ( $horaActMexico ) ) ;
	   $newdate = date ( 'Y-m-j H:i:s' , $newdate );
	   $horaActMexico= $newdate;
...
*******/
		...
		$sqlCli = "SELECT require_factura FROM anderp_clientes WHERE id_cliente=".$cliente[0];
		...
		if($numCli>0)
		{
			...
			{
				if($rowC[0] == 0){ //No requiere Factura
					$opcionCli = 2;
				}
			}
	    }
	...
	//Encabezado de Factura
	if($opcionCli == 1)
	{ //Requiere Factura Dejamos el mismo cliente de la nota de venta
		$sql = "SELECT 
				a.id_compania,
				a.id_sucursal,
				a.id_moneda,
	...
	else
	{
		...
		$sql = "SELECT 
				...
				SUM(a.total) as 'total'
				FROM anderp_notas_venta a 
			WHERE a.id_sucursal=".$IDSucursal."  AND  a.id_control_nota_venta IN(".$NVAsociadas.") group by a.id_compania ";  
