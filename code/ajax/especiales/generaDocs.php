<?php
	...
	function docElectronico($tabla, $llave)
	{
	   ...
	   $sql="SELECT
			  ...
			  'MEXICO',
			  cp
			  FROM anderp_companias c
			  LEFT JOIN anderp_ciudades cd ON c.id_ciudad = cd.id_ciudad
			  LEFT JOIN anderp_estados e ON cd.id_estado = e.id_estado
			  WHERE c.activo=1
			  AND c.id_compania=1";
		...
		//produccion
$sello=sello($cadenaOriginal, $ruta_llave_privada_key, $password,"/var/www/vhosts/sysandweb.com/ci/cache/");
...
//$sello=sello($cadenaOriginal, $ruta_llave_privada_key, $password,"/var/www/vhosts/sysandweb.com/subdomains/facelec/httpdocs/ci/cache/");
	...
	//	$sello=sello($cadenaOriginal, $ruta_llave_privada_key, $password, "C:/xampp/htdocs/Proyectos_SysandWeb/casaibarra/codigo/cache/");
		...
		$xml=xmlFE($datos, $sello, $cert, $no_certificado, $productos, $impuestos, $nombreFile, $ruta, $datosFiscales,$fechaDoc);
	...
	function getArrayDatos($tipo, $llave, $arr, $folio, $serie, $numero_aprobacion, $anio_aprobacion)
	{
		...
		$sql="SELECT
			...
			LEFT JOIN anderp_estados c ON b.id_localidad=c.id_estado
			left join anderp_ciudades d ON b.id_ciudad=d.id_ciudad
	...
	function getArrayProductos($tipo, $llave)
	{
		...
		$sql="SELECT
				format(IF(fd.id_unidad_venta=1,fd.kilos_netos,fd.cantidad),2) as cantidad,
				IF(fd.id_unidad_venta=1,'KG','Piezas') as 'unidad venta',
				p.id_producto,
				p.nombre, 
				format(fd.valor,2) as valor,
				format(fd.importe,2) as importe
				FROM  anderp_facturas_detalles fd
				JOIN anderp_productos p ON fd.id_producto = p.id_producto 
	...
	function getArrayDatosFiscales($tipoDoc, $llave){
		...
		$sql2="SELECT 
			...
			if($del_mpo_sucursal != ''){
			 ...
			 $newdatos[2] = str_replace("'","",$estado_sucursal);
             ...
             $sql4 = "SELECT regimen FROM anderp_companias WHERE id_compania=".$compania;
			  ...
			  //$newdatos[4] =  strtoupper($calle_no_sucursal.$aux_suc2);      //lugar de expedicion para cadena original
			  
			  $newdatos[4] =  str_replace("'","",$estado_sucursal);
