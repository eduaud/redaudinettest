...
extract($_POST);
if(isset($control)){
		$controlC=implode(',',json_decode($control));
		$where="cl_control_contratos_detalles.activo=1 AND cl_control_contratos_detalles.ultimo_movimiento=1 AND cl_contratos_acciones.id_accion_control IN (1,100,11) AND  cl_control_contratos.contrato='".$contrato."' AND cl_control_contratos_detalles.principal=1 AND id_detalle NOT IN(".$controlC.")";
}
else{
	$where="cl_control_contratos_detalles.activo=1 AND cl_control_contratos_detalles.ultimo_movimiento=1 AND cl_contratos_acciones.id_accion_control IN (1,100,11) AND cl_control_contratos_detalles.principal=1 AND cl_control_contratos.contrato='".$contrato."'";
}
$sql="
...
echo $smarty->fetch('especiales/agregaContrarecibosBusqueda.tpl');
