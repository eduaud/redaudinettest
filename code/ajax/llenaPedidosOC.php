...
if($pedido != ""){
		$where .= " AND id_pedido = '" . $pedido . "'";
		}
else{
		if($cliente != 0)
				$where .= " AND ad_clientes.id_cliente = " . $cliente;
		}
...
$sql = "SELECT id_control_pedido, id_pedido, 
		...
		WHERE IF(cantidad_entrada IS NULL, 0, cantidad_entrada) < cantidad_salida" . $where;
...
echo $smarty->fetch('especiales/respuestaPedidosOC.tpl');	
