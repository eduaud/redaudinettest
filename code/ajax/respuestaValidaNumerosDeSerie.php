		...
		for($i=0; $i<count($arreglo_errores); $i++){
			...
			for($j=0; $j<count($arreglo_errores); $j++){
				if($arreglo_errores[$i][0]==$arreglo_errores[$j][0]){
					$cadena_errores .= $arreglo_errores[$j][1].'<br />';
				}
			}
			$arreglo_compuesto_errores[$i][0] = $arreglo_errores[$i][0];
			$arreglo_compuesto_errores[$i][1] = $cadena_errores;
		}
		$errores .= '<div class="Table">';
		$errores .= '<div class="Title">E R R O R E S<img src="http://localhost/sistema_base/imagenes/ic18x18_tache.png" onClick="ocultaListaDeErrores();" align="right">&nbsp;&nbsp;</div>';
		$errores .= '<div class="Heading">';
		$errores .= '<div class="Cell">NO.</div>';
		$errores .= '<div class="Cell">NUMERO DE SERIE</div>';
		$errores .= '<div class="Cell">DESCRIPCION</div>';
		$errores .= '</div>';
		for($i=0; $i<count($arreglo_errores); $i++){
			$errores .= '<div id="renglon_error_'.$arreglo_compuesto_errores[$i][0].'" class="Row">';
			$errores .= '<div class="Cell">'.($i+1).'</div>';
			$errores .= '<div class="Cell">'.$arreglo_compuesto_errores[$i][0].'</div>';
			$errores .= '<div class="Cell">'.$arreglo_compuesto_errores[$i][1].'</div>';
			$errores .= '</div>';
		}
		$errores .= '</div>';
		echo $errores;
